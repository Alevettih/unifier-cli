...
export class AngularSpecifier extends Specifier {
  ...
  installMaterial(version: string): Promise<ExecaReturnValue> {
    return command(
      `npx --package @angular/cli ng add @angular/material@${version ? major(version) : 'latest'} --skip-confirmation --verbose`,
  ...
  installEsLint(version: string): Promise<ExecaReturnValue> {
    return command(
      `npx --package @angular/cli ng add @angular-eslint/schematics@${version ? major(version) : 'latest'} --skip-confirmation --verbose`,
      this.childProcessOptions
    );
  }
